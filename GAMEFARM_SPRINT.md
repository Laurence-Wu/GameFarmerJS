...
- Approach: Created `ItemRegistry.getItem(id)` utility function that returns `Element.getElementFromId(id) || Resource.getResource(id)`, replacing duplicated fallback pattern in 3 button files
- Files changed: src/game_manager/item_registry.js (+6 lines new), src/view/buttons/button_buy.js (-4 lines), src/view/buttons/button_sell.js (-4 lines), src/view/buttons/button_more.js (-3 lines)
- Lines added: 10, lines removed: 11, total delta: 21
- Deferred: none
